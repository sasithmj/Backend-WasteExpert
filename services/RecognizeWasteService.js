...
class RecognizeWasteService {
  static async recognizeWaste(photoPath) {
    try {
      ...
      const items = await detectItems(photoPath);
      return { success: true, message: "Image recognized", data: items };
    } catch (error) {
      throw new Error(`Error while recognizing waste: ${error}`);
    }
  }
}

module.exports = RecognizeWasteService;
